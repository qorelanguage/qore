...
QoreClass *initHTTPClientClass() {
   ...
   QC_HTTPCLIENT = new QoreClass("HTTPClient", QDOM_NETWORK);
   // no need to set the class synchronous flag because the QoreHTTPClient class is already thread-safe
   CID_HTTPCLIENT = QC_HTTPCLIENT->getID();

   QC_HTTPCLIENT->setConstructorExtended(HC_constructor, false, QC_NO_FLAGS, QDOM_DEFAULT);
   QC_HTTPCLIENT->setConstructorExtended(HC_constructor_hash, false, QC_NO_FLAGS, QDOM_DEFAULT, 1, hashTypeInfo, QORE_PARAM_NO_ARG);

   QC_HTTPCLIENT->setCopy((q_copy_t)HC_copy);
   QC_HTTPCLIENT->setDestructor((q_destructor_t)HC_destructor);

   QC_HTTPCLIENT->addMethodExtended("setHTTPVersion",         (q_method_t)HC_setHTTPVersion, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, stringTypeInfo, QORE_PARAM_NO_ARG);

   QC_HTTPCLIENT->addMethodExtended("getHTTPVersion",         (q_method_t)HC_getHTTPVersion, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, stringTypeInfo);

   QC_HTTPCLIENT->addMethodExtended("setSecure",              (q_method_t)HC_setSecure, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, softBoolTypeInfo, &True);

   QC_HTTPCLIENT->addMethodExtended("isSecure",               (q_method_t)HC_isSecure, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, boolTypeInfo);

   // returns either a string or nothing
   //*string HTTPClient::verifyPeerCertificate()  
   QC_HTTPCLIENT->addMethodExtended("verifyPeerCertificate",  (q_method_t)HC_verifyPeerCertificate, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, stringOrNothingTypeInfo);

   // returns either a string or nothing
   QC_HTTPCLIENT->addMethodExtended("getSSLCipherName",       (q_method_t)HC_getSSLCipherName, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, stringOrNothingTypeInfo);

   // returns either a string or nothing
   QC_HTTPCLIENT->addMethodExtended("getSSLCipherVersion",    (q_method_t)HC_getSSLCipherVersion, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, stringOrNothingTypeInfo);

   QC_HTTPCLIENT->addMethodExtended("connect",                (q_method_t)HC_connect, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo);

   QC_HTTPCLIENT->addMethodExtended("disconnect",             (q_method_t)HC_disconnect, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo);
   ...
   QC_HTTPCLIENT->addMethodExtended("send",                   (q_method_t)HC_send, false, QC_NO_FLAGS, QDOM_DEFAULT, hashTypeInfo, 5, dataTypeInfo, new BinaryNode, stringTypeInfo, QORE_PARAM_NO_ARG, stringTypeInfo, null_string(), hashTypeInfo, empty_hash(), softBoolTypeInfo, &False);
   QC_HTTPCLIENT->addMethodExtended("send",                   (q_method_t)HC_send, false, QC_NO_FLAGS, QDOM_DEFAULT, hashTypeInfo, 6, dataTypeInfo, new BinaryNode, stringTypeInfo, QORE_PARAM_NO_ARG, stringTypeInfo, null_string(), hashTypeInfo, empty_hash(), softBoolTypeInfo, &False, referenceTypeInfo, QORE_PARAM_NO_ARG);

   // *string HTTPClient::get(string $path, hash $headers = hash())  
   // *string HTTPClient::get(string $path, hash $headers = hash(), reference $ref)  
   QC_HTTPCLIENT->addMethodExtended("get",                    (q_method_t)HC_get, false, QC_NO_FLAGS, QDOM_DEFAULT, stringOrNothingTypeInfo, 2, stringTypeInfo, QORE_PARAM_NO_ARG, hashTypeInfo, empty_hash());
   QC_HTTPCLIENT->addMethodExtended("get",                    (q_method_t)HC_get, false, QC_NO_FLAGS, QDOM_DEFAULT, stringOrNothingTypeInfo, 3, stringTypeInfo, QORE_PARAM_NO_ARG, hashTypeInfo, empty_hash(), referenceTypeInfo, QORE_PARAM_NO_ARG);

   // hash HTTPClient::head(string $path, hash $headers = hash())  
   // hash HTTPClient::head(string $path, hash $headers = hash(), reference $info)  
   QC_HTTPCLIENT->addMethodExtended("head",                   (q_method_t)HC_head, false, QC_NO_FLAGS, QDOM_DEFAULT, hashTypeInfo, 2, stringTypeInfo, QORE_PARAM_NO_ARG, hashTypeInfo, empty_hash());
   QC_HTTPCLIENT->addMethodExtended("head",                   (q_method_t)HC_head, false, QC_NO_FLAGS, QDOM_DEFAULT, hashTypeInfo, 3, stringTypeInfo, QORE_PARAM_NO_ARG, hashTypeInfo, empty_hash(), referenceTypeInfo, QORE_PARAM_NO_ARG);

   // *string HTTPClient::post(string $path, data $data, hash $headers = hash())  
   // *string HTTPClient::post(string $path, data $data, hash $headers = hash(), reference $info)   
  QC_HTTPCLIENT->addMethodExtended("post",                   (q_method_t)HC_post, false, QC_NO_FLAGS, QDOM_DEFAULT, stringOrNothingTypeInfo, 3, stringTypeInfo, QORE_PARAM_NO_ARG, dataTypeInfo, QORE_PARAM_NO_ARG, hashTypeInfo, empty_hash());
   QC_HTTPCLIENT->addMethodExtended("post",                   (q_method_t)HC_post, false, QC_NO_FLAGS, QDOM_DEFAULT, stringOrNothingTypeInfo, 4, stringTypeInfo, QORE_PARAM_NO_ARG, dataTypeInfo, QORE_PARAM_NO_ARG, hashTypeInfo, empty_hash(), referenceTypeInfo, QORE_PARAM_NO_ARG);

   // nothing HTTPClient::setTimeout(timeout $timeout_ms = 0)  
   QC_HTTPCLIENT->addMethodExtended("setTimeout",             (q_method_t)HC_setTimeout, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, timeoutTypeInfo, zero());

   QC_HTTPCLIENT->addMethodExtended("getTimeout",             (q_method_t)HC_getTimeout, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, bigIntTypeInfo);

   QC_HTTPCLIENT->addMethodExtended("setEncoding",            (q_method_t)HC_setEncoding, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, stringTypeInfo, QORE_PARAM_NO_ARG);

   QC_HTTPCLIENT->addMethodExtended("getEncoding",            (q_method_t)HC_getEncoding, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, stringTypeInfo);

   QC_HTTPCLIENT->addMethodExtended("setURL",                 (q_method_t)HC_setURL, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, stringTypeInfo, QORE_PARAM_NO_ARG);

   QC_HTTPCLIENT->addMethodExtended("getURL",                 (q_method_t)HC_getURL, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, stringTypeInfo);

   // nothing HTTPClient::setProxyURL()  
   // clears the proxy URL
   QC_HTTPCLIENT->addMethodExtended("setProxyURL",            (q_method_t)HC_setProxyURL_nothing, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo);
   // nothing HTTPClient::setProxyURL(string $url)  
   QC_HTTPCLIENT->addMethodExtended("setProxyURL",            (q_method_t)HC_setProxyURL_str, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, stringTypeInfo, QORE_PARAM_NO_ARG);

   // *string HTTPClient::getProxyURL()  
   QC_HTTPCLIENT->addMethodExtended("getProxyURL",            (q_method_t)HC_getProxyURL, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, stringOrNothingTypeInfo);

   // nothing HTTPClient::clearProxyURL()  
   QC_HTTPCLIENT->addMethodExtended("clearProxyURL",          (q_method_t)HC_clearProxyURL, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo);

   // nothing HTTPClient::setProxySecure(softbool $b = True)  
   QC_HTTPCLIENT->addMethodExtended("setProxySecure",         (q_method_t)HC_setProxySecure, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, softBoolTypeInfo, &True);

   // bool HTTPClient::isProxySecure()  
   QC_HTTPCLIENT->addMethodExtended("isProxySecure",          (q_method_t)HC_isProxySecure, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, boolTypeInfo);

   // nothing HTTPClient::setMaxRedirects(softint $mr = 0)  
   QC_HTTPCLIENT->addMethodExtended("setMaxRedirects",        (q_method_t)HC_setMaxRedirects, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, softBigIntTypeInfo, zero());

   // int HTTPClient::getMaxRedirects()  
   QC_HTTPCLIENT->addMethodExtended("getMaxRedirects",        (q_method_t)HC_getMaxRedirects, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, bigIntTypeInfo);

   // nothing HTTPClient::setEventQueue()  
   QC_HTTPCLIENT->addMethodExtended("setEventQueue",          (q_method_t)HC_setEventQueue_nothing, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo);
   // nothing HTTPClient::setEventQueue(Queue $queue)  
   QC_HTTPCLIENT->addMethodExtended("setEventQueue",          (q_method_t)HC_setEventQueue_queue, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, QC_QUEUE->getTypeInfo(), QORE_PARAM_NO_ARG);

   // nothing HTTPClient::setConnectTimeout(timeout $timeout_ms = -1)  
   QC_HTTPCLIENT->addMethodExtended("setConnectTimeout",      (q_method_t)HC_setConnectTimeout, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 1, timeoutTypeInfo, new QoreBigIntNode(-1));

   // int HTTPClient::getConnectTimeout()  
   QC_HTTPCLIENT->addMethodExtended("getConnectTimeout",      (q_method_t)HC_getConnectTimeout, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, bigIntTypeInfo);

   // int HTTPClient::setNoDelay(softbool $b = True)  
   QC_HTTPCLIENT->addMethodExtended("setNoDelay",             (q_method_t)HC_setNoDelay, false, QC_NO_FLAGS, QDOM_DEFAULT, bigIntTypeInfo, 1, softBoolTypeInfo, &True);

   // bool HTTPClient::getNoDelay()  
   QC_HTTPCLIENT->addMethodExtended("getNoDelay",             (q_method_t)HC_getNoDelay, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, boolTypeInfo);

   // bool HTTPClient::isConnected()  
   QC_HTTPCLIENT->addMethodExtended("isConnected",            (q_method_t)HC_isConnected, false, QC_RET_VALUE_ONLY, QDOM_DEFAULT, boolTypeInfo);

   // nothing HTTPClient::setUserPassword(string user, string pass)
   QC_HTTPCLIENT->addMethodExtended("setUserPassword",        (q_method_t)HC_setUserPassword, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 2, stringTypeInfo, QORE_PARAM_NO_ARG, stringTypeInfo, QORE_PARAM_NO_ARG);
   // nothing HTTPClient::setUserPassword()   
   QC_HTTPCLIENT->addMethodExtended("setUserPassword",        (q_method_t)HC_clearUserPassword, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo);

   // nothing HTTPClient::clearUserPassword()
   QC_HTTPCLIENT->addMethodExtended("clearUserPassword",      (q_method_t)HC_clearUserPassword, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo);

   // nothing HTTPClient::setProxyUserPassword(string user, string pass)
   QC_HTTPCLIENT->addMethodExtended("setProxyUserPassword",   (q_method_t)HC_setProxyUserPassword, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo, 2, stringTypeInfo, QORE_PARAM_NO_ARG, stringTypeInfo, QORE_PARAM_NO_ARG);
   // nothing HTTPClient::setProxyUserPassword()   
   QC_HTTPCLIENT->addMethodExtended("setProxyUserPassword",   (q_method_t)HC_clearProxyUserPassword, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo);

   // nothing HTTPClient::clearProxyUserPassword()
   QC_HTTPCLIENT->addMethodExtended("clearProxyUserPassword", (q_method_t)HC_clearProxyUserPassword, false, QC_NO_FLAGS, QDOM_DEFAULT, nothingTypeInfo);

   return QC_HTTPCLIENT;
}
