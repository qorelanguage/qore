...
void qore_simple_tm::setDatePositive(int64 ty, int64 seconds) {
   ...
   for (int i = 1; ;i++) {
      //printd(5, "day=%d, qore_date_info::positive_months[%d]=%d, adjusted=%d\n", day, i, qore_date_info::positive_months[i], (qore_date_info::positive_months[i] + (ly && i > 1 ? 1 : 0)));
      if ((qore_date_info::positive_months[i] + (ly && i > 1 ? 1 : 0)) > day) {
	 //printd(5, "pm[%d]=%d pm[%d]=%d month=%d day=%d (%d)\n", i, qore_date_info::positive_months[i], i - 1, qore_date_info::positive_months[i - 1], i, day, day - qore_date_info::positive_months[i - 1] + 1);
	 month = i;
	 day = day - qore_date_info::positive_months[i - 1] + (ly && i > 2 ? 0 : 1);
	 break;
      }
   }
...
void qore_simple_tm::setDateNegative(int64 ty, int64 seconds) {
   //printd(0, "qore_simple_tm::setDateNegative(ty=%d, seconds=%lld)\n", ty, seconds);
   ...
   if (seconds)
      day--;
   bool ly = qore_date_info::isLeapYear(year);
   //printd(0, "1.1: seconds=%lld day=%d ly=%s\n", seconds, day, ly ? "true" : "false");
   // FIXME: this is inefficient - needs to be optimized
   for (int i = 1; ; i++) {
      printd(5, "nm[%d]=%d nm[%d]=%d adj=%d len=%d adj=%d day=%d (mon=%d day=%d)\n", i, qore_date_info::negative_months[i], i - 1, 
	     qore_date_info::negative_months[i - 1], (qore_date_info::negative_months[i - 1] - (ly && i == 12 ? 1 : 0)),
	     qore_date_info::month_lengths[13 - i], qore_date_info::month_lengths[13 - i] + (ly && (13 - i) == 2 ? 1 : 0),
	     day, 13 - i,
	     qore_date_info::month_lengths[13 - i] + (ly && (13 - i) == 2) + day - qore_date_info::negative_months[i - 1] + (ly && i == 12 ? 0 : 1));
      // check to find out what month we're in - add extra days for jan & feb if it's a leap year
      if ((qore_date_info::negative_months[i] - (ly && i > 10 ? 1 : 0)) <= day) {
	 month = 13 - i;
	 day = qore_date_info::month_lengths[month] + (ly && month == 2 ? 1 : 0) + day - qore_date_info::negative_months[i - 1] + (ly && i == 12 ? 2 : 1);
	 break;
      }
   }
   //printd(0, "2: seconds=%lld\n", seconds);
   hour = seconds / 3600;
   seconds -= hour * 3600;
   minute = seconds / 60;
   seconds -= minute * 60;
