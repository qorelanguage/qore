...
static void qore_socket_error(ExceptionSink *xsink, const char *err, const char *cdesc) {
   qore_socket_error_intern(WSAGetLastError(), xsink, err, cdesc); 
}
#else
// UNIX/Cygwin
...
static void qore_socket_error(ExceptionSink *xsink, const char *err, const char *cdesc) {
   qore_socket_error_intern(errno, xsink, err, cdesc); 
}
#endif
