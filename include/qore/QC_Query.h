...
class Query : public ReferenceObject
{
   ...
   public:
      ...
      inline ~Query();
      inline class Hash *exec(ExceptionSink *xsink);
      inline class Hash *check(ExceptionSink *xsink);
...
inline class Object *QPartNode::getQuery()
{
...
class QPartList
{
   ...
   public:
      int len;
      QPartNode **pl;
      inline QPartList(char *query, ExceptionSink *xsink);
      inline ~QPartList();
      inline QoreString *getSQL(ExceptionSink *xsink, int dyn);
};

#define QP_BLOCK 10
inline void QPartList::check_alloc()
{
   if (len == allocated)
   {
      allocated += QP_BLOCK;
...
inline void QPartList::addString(char *str, char *end)
{
   check_alloc();
   char s = *end;
   *end = '\0';
   char *ns = strdup(str);
   printd(5, "QPartList::addString() adding \"%s\"\n", ns);
   *end = s;
...
inline void QPartList::addQueryReference(char *&p, ExceptionSink *xsink)
{
   check_alloc();
   char *ep;
   if (!(ep = strchr(p, ':')))
...
inline void QPartList::addVariableReference(char *&p, ExceptionSink *xsink)
{
   check_alloc();
   char *ep = p + 1;
   ...
   char s = *ep;
   *ep = '\0';
   printd(5, "QPartList::addVariableReference() adding \"%s\"\n", p);
...
inline QPartList::QPartList(char *query, ExceptionSink *xsink)
{
   tracein("QPartList::QPartList()");
   ...
   char *p = query;
   char *last = query;
   printd(5, "QPartList::QPartList() \"%s\"\n", query);
   while (*p)
   {
      if (*p == '^' || *p == '~' || *p == '$')
      {
...
inline QoreString *QPartList::getSQL(ExceptionSink *xsink, int dyn)
{
   ...
   for (int i = 0; i < len; i++)
	 ...
	 if (n->type == NT_STRING)
	 {
	    printd(5, "QPartList::getSQL() adding node value \"%s\"\n",
	 ...
	 else if (n->type == NT_LIST)
	 {
	    for (int j = 0; j < n->val.list->size(); j++)
	    {
		  ...
		  if (v->type == NT_STRING)
		  {
		     printd(5, "QPartList::getSQL() adding list %d/%d node value \"%s\"\n",
			    j, n->val.list->size(),
   ...
   printd(5, "QPartList::getSQL() (len=%d) returning \"%s\"\n", 
	  len, str->getBuffer());
   return str;
}
...
inline class Hash *Query::exec(ExceptionSink *xsink)
{
   if (!qpl)
      return NULL;
   ...
   if (xsink->isEvent())
      return NULL;
   class Hash *h = datasource->select(str->getBuffer(), xsink);
   delete str;
   return h;
}

inline class Hash *Query::check(ExceptionSink *xsink)
{
   if (!qpl)
      return NULL;
   class QoreString *str = qpl->getSQL(xsink, isDynamic());
