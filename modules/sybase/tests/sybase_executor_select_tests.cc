...
static void create_bit_table()
{
  ...
  cmd = "insert into bit_table values (1)";
  sybase_low_level_execute_directly_command(c.getConnection(), cmd, &xsink);
  if (xsink.isException()) {
    assert(false);
  }
  cmd = "insert into bit_table values (0)";
  sybase_low_level_execute_directly_command(c.getConnection(), cmd, &xsink);
  if (xsink.isException()) {
    assert(false);
  }
...
TEST()
{
  ...
  n = n->val.list->retrieve_entry(0);
  assert(n->type == NT_HASH);
  n = n->val.hash->getKeyValue("column1");
  assert(n);
  assert(n->type == NT_BOOLEAN);
  assert(n->val.boolval);
}
...
// TBD - add decimal, numeric, binaries tests
