...
namespace sybase_tests_76523761267110 {
...
TEST()
{
  ...
  delete_datetime_table(true);
  create_datetime_table();
  ON_BLOCK_EXIT(delete_datetime_table, false);

  sybase_executor executor;
  executor.m_parsed_query.m_result_dynamic_query_text = "select * from datetime_table where datetime_col = ?";
...
TEST()
{
  ...
  sybase_executor executor;
  executor.m_parsed_query.m_result_dynamic_query_text = "insert into datetime_table values (?)";
  executor.m_parsed_query.m_is_procedure = false;
  ...
  executor.m_parsed_query.m_result_dynamic_query_text = "select count(*) from datetime_table";
  ...
  executor.m_parsed_query.m_result_dynamic_query_text = "select * from datetime_table";
  ...
  x = x->val.hash->getKeyValue("datetime_col");
  assert(x);
  ...
  executor.m_parsed_query.m_result_dynamic_query_text = "delete from datetime_table";
  executor.m_args = new List;
