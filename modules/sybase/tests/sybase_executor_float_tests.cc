...
namespace sybase_tests_818752765 {
...
TEST()
{
  ...
  delete_float_table(true);
  create_float_table();
  ON_BLOCK_EXIT(delete_float_table, false);

  sybase_executor executor;
  executor.m_parsed_query.m_result_dynamic_query_text = "select * from float_table where float_col = ?";
...
TEST()
{
  ...
  sybase_executor executor;
  executor.m_parsed_query.m_result_dynamic_query_text = "insert into float_table values (?)";
  executor.m_parsed_query.m_is_procedure = false;
  ...
  executor.m_parsed_query.m_result_dynamic_query_text = "insert into float_table values (1.2)";
  ...
  executor.m_parsed_query.m_result_dynamic_query_text = "select count(*) from float_table";
  ...
  executor.m_parsed_query.m_result_dynamic_query_text = "select * from float_table";
  ...
  x = x->val.hash->getKeyValue("float_col");
  assert(x);
  assert(x->type == NT_FLOAT);
  double delta = fabs(x->val.floatval - 1.2);
  assert(delta < 0.001);

  executor.m_parsed_query.m_result_dynamic_query_text = "delete from float_table";
  executor.m_args = new List;
  n = executor.exec(&xsink);
  if (xsink.isException()) {
    assert(false);
