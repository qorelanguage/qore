...
class T {
   ...
   private:
	 ...
	 methodMap.addMethod(new DynamicSlot(qore_obj, 0, 0));
   ...
   public:
	 ...
	 qore_qt_method_map_t::iterator i = methodMap.find(id);
	 //printd(5, "%s::qt_metacall() id=%d (%s)\n", metaObject()->className(), id, i == methodMap.end() ? "not found" : (i->second->getSlot() ? "slot" : "signal"));
	 if (i == methodMap.end())
	    return -1;

	 DynamicSlot *slot = i->second->getSlot();
	 if (slot)
	    slot->call(sender(), arguments);
	 else {
	    //QoreQtDynamicSignal *ds = static_cast<QoreQtDynamicSignal *>(i->second);
	    //if (!ds->targ)
	    // return -1;

	    //printd(5, "%s::qt_metacall() activating signal %d ()\n", metaObject()->className(), id); 
	 ...
	 bool is_dynamic;
	 int targetId = is_signal ? getSignalIndex(theSlot, is_dynamic) : getSlotIndex(theSlot, xsink);
	 if (targetId < 0) {
	    if (!*xsink)
	       if (is_signal)
	 ...
	 is_dynamic = false;
	 int signalId = sender->getSignalIndex(theSignal, is_dynamic);
	 if (signalId < 0) {
	 ...
	 return 0;
      ...
      DLLLOCAL int createDynamicSignal(const char *signal)
      {
	 QByteArray theSignal = QMetaObject::normalizedSignature(signal);
	 int signalId = signalIndices.value(theSignal, -1);
	 if (signalId >= 0)
	    return -1;
	 signalId = methodMap.addMethod();
	 ...
	 DynamicSlot *ds = new DynamicSlot(qore_obj, theSlot.data(), xsink);
	 if (*xsink) {
	    delete ds;
	    return -1;
	 }
	 ...
	 return slotId + metaObject()->methodCount();
      ...
      DLLLOCAL int getSignalIndex(const QByteArray &theSignal, bool &is_dynamic) const
      {
	 int signalId = metaObject()->indexOfSignal(theSignal);
