...
static void QKEYSEQUENCE_constructor(class QoreObject *self, const QoreNode *params, ExceptionSink *xsink)
{
   ...
   if (is_nothing(p))
      ...
      qks = new QoreQKeySequence(p->val.String->getBuffer());
   else {
      int i = p->getAsInt();
...
//uint count () const
static QoreNode *QKEYSEQUENCE_count(QoreObject *self, QoreQKeySequence *qks, const QoreNode *params, ExceptionSink *xsink)
{
   return new QoreNode((int64)qks->count());
}

//DataPtr & data_ptr ()
//static QoreNode *QKEYSEQUENCE_data_ptr(QoreObject *self, QoreQKeySequence *qks, const QoreNode *params, ExceptionSink *xsink)
//{
//   ??? return new QoreNode((int64)qks->data_ptr());
//}

//bool isEmpty () const
static QoreNode *QKEYSEQUENCE_isEmpty(QoreObject *self, QoreQKeySequence *qks, const QoreNode *params, ExceptionSink *xsink)
{
   return new QoreNode(qks->isEmpty());
}

//SequenceMatch matches ( const QKeySequence & seq ) const
static QoreNode *QKEYSEQUENCE_matches(QoreObject *self, QoreQKeySequence *qks, const QoreNode *params, ExceptionSink *xsink)
{
   QoreNode *p = get_param(params, 0);
...
static QoreNode *QKEYSEQUENCE_toString(QoreObject *self, QoreQKeySequence *qks, const QoreNode *params, ExceptionSink *xsink)
{
   QoreNode *p = get_param(params, 0);
   QKeySequence::SequenceFormat format = (QKeySequence::SequenceFormat)(p ? p->getAsInt() : 0);
   return new QoreNode(new QoreString(qks->toString(format).toUtf8().data(), QCS_UTF8));
}
