...
if ($db)
{
    $s1 = new Datasource(SQL::DSOracle, "uiautomation", "uiautomation", "websht01");
    $s2 = new Datasource(SQL::DSOracle, "uiautomation", "uiautomation", "websht01");
    $s1.open();
    $s2.open();
}
...
/*
    if ($db)
    {
	my $q = $ds.execSelect("select entryid from spoolerentries");
	foreach my $e in ($q.entryid)
	    printf("TID:%d DS:%s %s %s\n", gettid(), $ds, $arg, $e);
    }
*/
...
#printf("thread 1 TID: %d\n", background gee("geronimo", $db, $delay));
